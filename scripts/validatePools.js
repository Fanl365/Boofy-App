...
const overrides = {
  ...
  'heco-bifi-maxi': { boofyFeeRecipient: undefined }, // 0x0
  'polygon-bifi-maxi': { boofyFeeRecipient: undefined }, // 0x0
  'avax-bifi-maxi': { boofyFeeRecipient: undefined }, // 0x0
  'bifi-maxi': { stratOwner: undefined }, // harvester 0xDe30
  'beltv2-4belt': { vaultOwner: undefined }, // moonpot deployer
  'cronos-bifi-maxi': { boofyFeeRecipient: undefined }, // 0x0
  'metis-bifi-maxi': { boofyFeeRecipient: undefined }, // 0x0
  'aurora-bifi-maxi': { boofyFeeRecipient: undefined }, // 0x0
  'fuse-bifi-maxi': { boofyFeeRecipient: undefined }, // 0x0
  'moonbeam-bifi-maxi': { boofyFeeRecipient: undefined }, // 0x0
  'scream-frax': { vaultOwner: undefined }, // Rescue
};

const oldValidOwners = [
  addressBook.fantom.platforms.boofyfinance.devMultisig,
  addressBook.polygon.platforms.boofyfinance.devMultisig,
  addressBook.arbitrum.platforms.boofyfinance.devMultisig,
];
...
const validatePools = async () => {
  ...
  for (let [chain, pools] of Object.entries(chainPools)) {
    ...
    pools.forEach(pool => {
      ...
      const { keeper, strategyOwner, vaultOwner, boofyFeeRecipient } =
        addressBook[chain].platforms.boofyfinance;
      ...
      updates = isBoofyFeeRecipientCorrect(pool, chain, boofyFeeRecipient, updates);
    });
    if (!isEmpty(updates)) {
      exitCode = 1;
    }
...
const isBoofyFeeRecipientCorrect = (pool, chain, recipient, updates) => {
  const validRecipients = oldValidFeeRecipients[chain] || [];
  if (
    pool.status === 'active' &&
    pool.boofyFeeRecipient !== undefined &&
    pool.boofyFeeRecipient !== recipient &&
    !validRecipients.includes(pool.boofyFeeRecipient)
  ) {
    console.log(
      `Pool ${pool.id} should update boofy fee recipient. From: ${pool.boofyFeeRecipient} To: ${recipient}`
    );

    if (!('boofyFeeRecipient' in updates)) updates['boofyFeeRecipient'] = {};
    if (!(chain in updates.boofyFeeRecipient)) updates.boofyFeeRecipient[chain] = {};

    if (pool.stratOwner in updates.boofyFeeRecipient[chain]) {
      updates.boofyFeeRecipient[chain][pool.stratOwner].push(pool.strategy);
    } else {
      updates.boofyFeeRecipient[chain][pool.stratOwner] = [pool.strategy];
    }
  }
...
const populateVaultsData = async (chain, pools, web3) => {
  const multicall = new MultiCall(web3, addressBook[chain].platforms.boofyfinance.multicall);
...
const populateStrategyData = async (chain, pools, web3) => {
  ...
  const calls = pools.map(pool => {
    ...
    return {
      ...
      boofyFeeRecipient: stratContract.methods.boofyFeeRecipient(),
  ...
  return pools.map((pool, i) => {
    return {
      ...
      boofyFeeRecipient: results[i].boofyFeeRecipient,
